...
def parse_chunk_id(chunk_id: str) -> tuple[str, str, int] | None:
    """Parse chunk_id into (path, name, lineno); None при неверном формате."""
...
def chunks_match(predicted: str, reference: str, tolerance: int = 2) -> bool:
    """Проверяет совпадение predicted и reference в пределах ±tolerance строк.

    path и name сверяются точно (с учётом регистра), start_line - с допуском tolerance.
    """
...
def score_question(top5: list[str], correct: list[str]) -> float:
    """Precision@5 для одного вопроса: matched / min(5, len(correct)).

    Нормировка даёт 1.0 при нахождении всех reference-чанков независимо от их числа.
    """
    if len(correct) == 0:
...
def load_json(path: Path, label: str) -> list | None:
    """Читает и парсит JSON-файл; None при ошибке."""
    if not path.exists():
...
def main() -> int:
    """Считает Precision@5 по predictions/questions и печатает отчёт; код возврата 0/1."""
